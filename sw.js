...
// Scope: cache the app shell (this page + manifest + icons) AND the Quran/tafsir
// text (alquran.cloud) so the Quran actually opens and reads offline. Reciter
// audio (mp3quran.net, cdn.islamic.network) and prayer-time lookups (aladhan.com)
// are left completely untouched and always go straight to the network — audio is
// too large to auto-store for every reciter, and prayer times must stay fresh.
// The one exception is the Azkar reciter's audio (hisnmuslim.com), handled in its
// own permanent cache further below, since that one was explicitly requested to
// work offline too.
...
const CACHE_NAME = 'noor-alquran-shell-v6';
const SHELL_FILES = [
  './',
  './index.html',
...
// and prayer-time data need a real network request every time.
  ...
  event.waitUntil(
    caches.keys()
      .then((names) => Promise.all(
        names
          .filter((n) => n !== CACHE_NAME && n !== AZKAR_AUDIO_CACHE && n !== QURAN_TEXT_CACHE)
